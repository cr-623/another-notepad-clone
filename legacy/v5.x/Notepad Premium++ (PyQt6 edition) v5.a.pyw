...
from PyQt6.QtWidgets import (
    ...
    QMenuBar, QSpinBox, QDialogButtonBox
)
from PyQt6.QtGui import (
    QFont, QAction, QKeySequence, QIcon, QTextCursor, QTextCharFormat,
...
class SyntaxHighlighter(QSyntaxHighlighter):
    ...
    def setup_formats(self):
        ...
        # Define colors based on mode and theme
        if self.mode == "code":
            if self.theme == "dark":
                colors = {
                    'number': '#445ad4',
                    ...
                    'bracket': '#1400eb',
                    'bracket_text': '#7da8ff',
                    'bracket_err': ('#ef4444', '#ffffff'),  # (bg, fg)
                    'punct': '#484848'
                }
            else:  # light theme
                colors = {
    ...
    def highlightBlock(self, text):
        ...
        # Variables (identifiers not followed by parenthesis)
        var_pattern = QRegularExpression(r'\b[a-zA-Z_][a-zA-Z0-9_]*\b')
        iterator = var_pattern.globalMatch(text)
        while iterator.hasNext():
            match = iterator.next()
            # Check if not already formatted (as function or number)
    ...
    def highlight_brackets(self, text):
        """Highlight matching and mismatched brackets."""
        stack = []
        pairs = {')': '(', ']': '[', '}': '{'}
...
class CodeEditor(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.current_theme = "light"  # Track current theme for line numbers
    ...
    def line_number_area_paint_event(self, event):
        ...
        try:
            ...
            # Silently handle painting errors to prevent crashes
...
class TabData:
    ...
    last_saved_content: str = ""  # Track last autosaved content
...
class FindDialog(QDialog):
    ...
    def __init__(self, parent, text_widget, app):
        ...
        # Inherit parent's stylesheet for theme consistency
        if parent:
...
class NotepadApp(QMainWindow):
    def __init__(self):
        ...
        # Apply theme to the application FIRST before creating tabs
    ...
    def _init_ui(self):
        ...
        # Enable middle-click to close (requires event filter)
    ...
    def eventFilter(self, obj, event):
        """Event filter for middle-click tab closing."""
    ...
    def _setup_shortcuts(self):
        ...
        next_tab_action.setShortcut(next_tab_shortcut)
        ...
        prev_tab_action.setShortcut(prev_tab_shortcut)
        ...
        try:
            ...
            for i in range(self.tab_widget.count()):
                ...
                if tab_id and tab_id in self.tabs:
                    ...
                    # Get cursor and scroll positions
                    cursor = text_widget.textCursor()
                    cursor_pos = cursor.position()
                    scroll_pos = text_widget.verticalScrollBar().value()
    ...
    def _restore_session(self) -> bool:
        """Restore the previous session. Returns True if session was restored."""
        try:
            ...
            for tab_session_dict in app_session.tabs:
                ...
                tab_id = self._new_tab(
                    filename=tab_session.current_file,
                    content=tab_session.content
                )
    ...
    def _clear_session(self):
        ...
        try:
            if os.path.exists(SESSION_FILE):
                ...
                QMessageBox.information(self, "Clear Session", 
                                       "Saved session has been cleared.")
            else:
                QMessageBox.information(self, "Clear Session", 
    ...
    def _toggle_restore_session(self):
        """Toggle the restore session setting."""
    ...
    def _autosave_settings(self):
        """Open dialog to configure autosave settings."""
        dialog = QDialog(self)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if self.preferences.autosave_enabled:
                ...
                if not old_enabled:
                    ...
                    QMessageBox.information(self, "Autosave", 
                                          f"Autosave interval changed to {self.preferences.autosave_interval} seconds.")
            else:
                self.autosave_timer.stop()
                if old_enabled:
    ...
    def _autosave_all(self):
        """Autosave all tabs that have associated files."""
        saved_count = 0
        
        for tab_id, tab_data in self.tabs.items():
            # Only autosave if:
            # 1. Tab has a file associated with it
            # 2. Content has changed since last save
    ...
    def _new_tab(self, filename=None, content=""):
        ...
        # Create text editor
        text_widget = CodeEditor()
        text_widget.setFont(QFont(self.preferences.font_family, self.current_font_size))
        ...
        # Apply theme to the new tab FIRST (before adding to widget or setting content)
        ...
        # Apply syntax highlighting (must be done BEFORE setting content for proper highlighting)
        text_widget.set_syntax_highlighting(self.highlight_syntax, self.preferences.theme)
        
        # Insert content if provided (after theme is applied)
        if content:
    ...
    def _get_tab_id_from_index(self, index):
        """Get tab ID from tab widget index."""
    ...
    def _apply_theme(self, theme: str):
        """Apply color theme to the editor and entire application."""
        # Apply to all text editor tabs
        for tab_data in self.tabs.values():
            self._apply_theme_to_tab(tab_data, theme)
        
        # Apply application-wide styling (both dark and light modes now have consistent tab styling)
        if theme == "dark":
            dark_stylesheet = """
            ...
            # Light theme with rounded tabs matching dark mode
            light_stylesheet = """
            QTextEdit {
    ...
    def _update_recent_files_menu(self):
        ...
        if not self.preferences.recent_files:
            ...
            # Map numbers 1-9 to shortcuts, then A-F for 10-15
    ...
    def _recent_files_settings(self):
        """Open dialog to configure recent files settings."""
        dialog = QDialog(self)
    ...
    def _find_next(self):
        ...
        if not self.last_find_term:
            QMessageBox.information(self, "Find Next", 
                                   "Use Ctrl+F to open Find dialog first.")
            return
    ...
    def _evaluate_formula(self):
        ...
        for line in lines:
            ...
            if stripped.endswith('='):
                ...
                # Variable assignment detection (simplified version)
                if ':' in content:
    ...
    def _list_variables(self):
        """Show a dialog with all currently defined variables."""
        tab_data = self._get_current_tab()
        if not tab_data:
            return
        ...
        QMessageBox.information(self, "Variable Inspector", help_text)
    ...
    def _font_dialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            for tab_data in self.tabs.values():
                font = tab_data.text_widget.font()
                font.setPointSize(self.current_font_size)
                tab_data.text_widget.setFont(font)
            self._save_preferences()
    ...
    def _change_theme(self, theme: str):
        ...
        # Apply theme to application-wide UI (menus, dialogs, etc.)
