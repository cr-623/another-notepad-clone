...
from PyQt6.QtWidgets import (
    ...
    QMenuBar, QSpinBox, QDialogButtonBox, QFontComboBox, QListWidget,
    QListWidgetItem, QSplitter, QFrame
)
from PyQt6.QtGui import (
    QFont, QFontDatabase, QAction, QKeySequence, QIcon, QTextCursor, QTextCharFormat,
    QColor, QPalette, QSyntaxHighlighter, QTextDocument, QActionGroup, QWheelEvent
)
from PyQt6.QtCore import Qt, QRegularExpression, pyqtSignal, QTimer, QSize
...
@dataclass
class ClosedTab:
    """Data for a recently closed tab (for reopen)."""
    title: str
    content: str
...
class CodeEditor(QTextEdit):
    """Text editor with line numbers, auto-evaluation, RMB-scroll zoom, and boundary navigation."""
    ...
    def __init__(self, parent=None):
        ...
        self._rmb_held = False          # ← RMB zoom tracking
        self._rmb_zoom_accum = 0        # accumulated delta for smooth zoom
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.RightButton:
            if self._rmb_held and self._rmb_zoom_accum == 0:
                # No scroll happened → show context menu
    ...
    def wheelEvent(self, event: QWheelEvent):
        if self._rmb_held and self._notepad_app is not None:
            ...
            # Each 120-unit tick = one zoom step
            steps = self._rmb_zoom_accum // 120
            if steps != 0:
                self._rmb_zoom_accum -= steps * 120
                if steps > 0:
                    for _ in range(abs(steps)):
                        self._notepad_app._zoom_in()
                else:
                    for _ in range(abs(steps)):
                        self._notepad_app._zoom_out()
            event.accept()
            return
        super().wheelEvent(event)

    def _show_context_menu(self, global_pos):
        """Show a standard right-click context menu."""
        menu = QMenu(self)
        ...
        if self._notepad_app:
            menu.addSeparator()
            menu.addAction("Evaluate Formula\tCtrl+E", self._notepad_app._evaluate_formula)
...
# ---------------------------------------------------------------------------
# Font Dialog  (redesigned with live preview panel + filter)
# ---------------------------------------------------------------------------

class FontDialog(QDialog):
    """Improved font picker: searchable list, live preview, size spinner."""
    ...
    def _update_preview(self):
        font = QFont(self.selected_family, self.size_spin.value())
...
class NotepadApp(QMainWindow):
    MAX_CLOSED_TABS = 20  # how many closed tabs to remember
    ...
    def __init__(self, files_to_open=None):
        ...
        self.tab_counter = 0
        self.closed_tabs: List[ClosedTab] = []   # ← reopen closed tabs history
        ...
        # Open files passed on the command line, otherwise restore session
        if files_to_open:
    ...
    # =========================================================================
    # HELPER: open a file path directly (used by argparse and recent files)
    # =========================================================================

    def _open_file_path(self, filepath: str):
        """Open a file by absolute or relative path into a new tab."""
    ...
    def _create_menus(self):
        ...
        # ── Reopen Closed Tab ──────────────────────────────────────────
        self.reopen_action = QAction("&Reopen Closed Tab\tCtrl+Shift+T", self)
        ...
        # Reopen history submenu
        self.reopen_menu = QMenu("Reopen &History", self)
        file_menu.addMenu(self.reopen_menu)
        # ──────────────────────────────────────────────────────────────
        ...
        # ── Fullscreen ────────────────────────────────────────────────
        self.fullscreen_action = QAction("&Full Screen\tF11", self)
    ...
    def keyPressEvent(self, event):
        # Allow Escape or F11 to exit fullscreen
    ...
    def _push_closed_tab(self, tab_data: TabData, tab_id: int):
        """Save a snapshot of a tab before it is closed."""
        text_widget = tab_data.text_widget
        cursor = text_widget.textCursor()
        title = (
            os.path.basename(tab_data.current_file)
            if tab_data.current_file
            else f"Untitled {tab_id}"
        )
        closed = ClosedTab(
            title=title,
    ...
    def _reopen_closed_tab(self):
        if not self.closed_tabs:
            return
        closed = self.closed_tabs.pop()
        self._restore_closed_tab(closed)
    ...
    def _close_tab(self, tab_id=None):
        ...
        # Push to closed-tabs history BEFORE removing
        self._push_closed_tab(tab_data, tab_id)
    ...
    def _font_dialog(self):
        dialog = FontDialog(
            self,
            current_family=self.active_font_family,
            current_size=self.current_font_size,
            theme=self.preferences.theme,
        )
        if dialog.exec() == QDialog.DialogCode.Accepted:
            family, size = dialog.get_result()
...
Reopen Closed Tabs:
  ...
  File > Reopen History → browse all closed tabs

Fullscreen:
  F11 → toggle fullscreen (hides menu & tab bar)
  Esc or F11 → exit fullscreen

Command Line:
  notepad.py file1.txt file2.txt  → open files directly
  (also works via double-click in file manager)"""
        QMessageBox.information(self, "Formula Help", help_text)
    
    def _show_about(self):
        about_text = """Enhanced Notepad  —  Version 5.3 (PyQt6)

EDITING
  • Unlimited tabbed documents
  • Full undo / redo history per tab
  • Find, Find Next/Previous (F3/Shift+F3)
...
FORMULA CALCULATOR
  • Type any expression ending in '=' → auto-evaluates
  • Ctrl+E → evaluate all formulas in document/selection
  • Named variables  (x = 5+3  stores x)
  • Colon syntax  (area: w * h)
  • Functions: sqrt, sin, cos, tan, log, log10, exp,
    abs, round, min, max, sum, pow
  • Constants: pi, e
  • Variable inspector & clear (Edit menu)

FILES & SESSION
  • Open / Save / Save As / Save All
  • Autosave (configurable interval, named files only)
  • Session restore — reopens all tabs on next launch
  • Recent files list (up to 20, keyboard accessible)
  • Reopen closed tabs — Ctrl+Shift+T or File > Reopen History
  • New Window (Ctrl+Shift+N)
  • Command-line / double-click file opening
    (e.g. notepad.py file1.txt file2.txt)
...
APPEARANCE
  ...
  • Line numbers toggle
  • Syntax highlighting — Off / Code / Text modes
  • Status bar with line & column (toggleable)
  • Fullscreen mode — F11 (hides menu & tab bar)
    Esc or F11 to exit

NAVIGATION
  • Tab switching — Ctrl+Tab / Ctrl+Shift+Tab
  • Middle-click tab to close
  • Up/Down boundary teleport (first/last line wrap)
...
Built with Python 3 and PyQt6"""
    ...
    def closeEvent(self, event):
        # Snapshot session first so all open tabs are captured
        self._save_session()
        for tab_id in list(self.tabs.keys()):
...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced Notepad — a feature-rich tabbed text editor.",
        epilog="You can also associate this script with file types so double-clicking opens files directly.",
    )
    parser.add_argument(
        "files",
        nargs="*",
        metavar="FILE",
        help="One or more files to open on startup.",
    )
    # parse_known_args so Qt's own argv flags (like -style, -platform) don't cause errors
    args, qt_args = parser.parse_known_args()

    # Rebuild argv for QApplication with only unrecognised (Qt) args
    qt_argv = [sys.argv[0]] + qt_args
    app = QApplication(qt_argv)

    files_to_open = [f for f in args.files if f]  # drop empty strings
    window = NotepadApp(files_to_open=files_to_open if files_to_open else None)
    window.show()
    sys.exit(app.exec())
